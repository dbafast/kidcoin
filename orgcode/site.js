//
// Copyright (c) 2013 Kidcoin Developers
//
...
if(fs.existsSync(__dirname + '/certificates')) {
    ENABLE_SSL = true;
    var certificates = {
        key : fs.readFileSync(__dirname + '/certificates/kidcoin.org.key').toString(),
        cert : fs.readFileSync(__dirname + '/certificates/kidcoin.org.crt').toString(),
        ca : [ ]
    }
...
function Stats() {
    ...
    self.push = function(headers, url) {
        ...
        if(stats.referer_count < (1024 * 16) && headers.referer && headers.referer.indexOf('https://kidcoin.org') == -1) {
...
*/
    app.get('/upgrade', function(req, res) {
        res.redirect("http://blog.kidcoin.org/2013/12/kidcoin-086-release-notes.html");
    });
    ...
    function init() {
        ...
        app.get('/robots.txt', function(req, res) {
            res.end("Sitemap: http://kidcoin.org/sitemap.xml");
        })
...
//       ENABLE_SSL = false;
        ...
        if(!ENABLE_SSL) {
            ...
            unsecure.configure(function(){
                unsecure.use('/', function(req, res, next) {
                    res.redirect("https://kidcoin.org"+req.originalUrl);
                })
            })            
    ...
    function secure() {
        if(process.platform != 'win32') {
            try {
                exec('id -u kidcoin', function(err, stdout, stderr) {
                    if(!err) {
