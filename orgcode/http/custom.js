//
// Copyright (c) 2013 Kidcoin Project
//
...
try {
	...
	if(typeof(sessionStorage) != 'undefined') {
		if(!sessionStorage['kidcoin-locale']) {
			sessionStorage.setItem('kidcoin-locale', true);
...
function Popup(key, voffset) {
	...
	function make_visible(show) {
		if(show && !popup_visible) {
			$("#kidcoin-"+key).css({visibility:'visible'});
			$("#kidcoin-"+key).fadeTo("fast", 1.0);
			popup_visible = true;
		}
		else
		if(popup_visible)
		{
			$("#kidcoin-"+key).css({ display : 'none', 'opacity' : 0 });	
			$("#kidcoin-"+key).css({visibility:'hidden'});
			popup_visible = false;
			// mouse_local = false;
		}
	}
	...
	var rect = null;
	$("#kidcoin-"+key).mouseenter(function() {
		mouse_local = true;
		if(cleanup_timeout) {
